...
from color.term_colors import Color

ASCII_CHARS = [ '#', '?', ' ', '.', '=', '+', '.', '*', '3', '&', '@']
# ASCII_CHARS = [
#     f'{Color.BLUE}#{Color.END}', 
#     f'{Color.RED}?{Color.END}', 
#     ' ',
#     f'{Color.GREEN}.{Color.END}',
#     f'{Color.BLUE}={Color.END}',
#     f'{Color.WHITE}+{Color.END}',
#     f'{Color.WHITE}.{Color.END}',
#     f'{Color.WHITE}*{Color.END}',
#     f'{Color.WHITE}3{Color.END}',
#     f'{Color.BLUE}&{Color.END}',
#     f'{Color.BLUE}@{Color.END}']
...
def handle_image_conversion(image_filepath, clearity):
    ...
    try:
        ...
        print(f"{Color.RED}Unable to open image file {image_filepath}.{Color.RED}")
        print(f"{Color.RED}{e}{Color.END}")
        return

    image_ascii = convert_image_to_ascii(image, clearity)
    # print(image_ascii)
    colored_ascii = Color().color_full_string(image_ascii)
    print(colored_ascii)
...
def create_thumbnail(image_file_path):
    ...
    try:
        ...
        print(f"{Color.RED}Unable to open image file {image_file_path}.{Color.RED}")
        print(e)
        return
    ...
    print(f"{Color.GREEN}Thumbnail created. Please check in the current directory.{Color.END}")
...
def menu():
    ...
    if choice == "A" or choice == "a":
        print(f"\n {Color.GREEN}Creating an ASCII representation of {image_file_path}:{Color.END} \n")
